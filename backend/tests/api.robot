...
Verify Article Lifecycle - Create, Delete
    ...
    Log To Console    \nSuccessfully created and deleted ID: ${id}
...
Verify Delete Article Request - Non Existent Article Validation
    ${headers}=    Create Dictionary    Content-Type=application/json
    ...
    #as biggest id corresponding to an article +1, or 1 if there are no articles
    ...
    ELSE
        Log To Console   List is empty. Using default ID 1.
        ${non_existent_id}=  Set Variable    1
    END
    ...
    #2. Define the expected error dictionary
    ${expected_error}=    Create Dictionary    detail=${{ 'Not found.' }}

    #3. Compare the actual JSON response to the expected dictionary
    Dictionaries Should Be Equal    ${del_response.json()}    ${expected_error}
    
    Log To Console    \nResponse verified successfully: ${del_response.json()} for ID: ${non_existent_id}

Verify Delete Articles Request - Method Not Allowed Validation
    ${headers}=    Create Dictionary    Content-Type=application/json

    #1. Try to delete all articles
    ${del_response}=    DELETE    ${BASE_URL}   headers=${headers}   expected_status=405

    ${expected_error}=    Create Dictionary    detail=${{ 'Method "DELETE" not allowed.' }}

    #2. Compare the actual JSON response to the expected dictionary
    Dictionaries Should Be Equal    ${del_response.json()}    ${expected_error}
    
    Log To Console    \nResponse verified successfully: ${del_response.json()}

Verify Get Article Request
    ${headers}=  Create Dictionary  Content-Type=application/json
...
Verify Get Article Requests - Data Consistency
    ${headers}=    Create Dictionary    Content-Type=application/json
...
Verify Article Lifecycle - Create, Read, Delete
    ${headers}=    Create Dictionary    Content-Type=application/json
    #1. Set a unique article name for purposes of this particular test
    ${article_title}=    Set Variable    Unique Lifecycle Test Article 4
    ...
    ${article_id}=    Convert To String   ${resp_post.json()}[id]
...
#Verify Patch Article Request - Non Existent Article Validation
#    ${headers}=    Create Dictionary    Content-Type=application/json
#
#    #1. Get the list of all articles
#    ${response}=   GET    ${BASE_URL}   headers=${headers}  expected_status=200
...
#    ${non_existent_id}=    Evaluate    ${max_id} + 1
#    
#    Log To Console    \nTrying to Patch ID: ${non_existent_id}
#
#    #3. Try to dpatch article with {non_existent_id}
#    ${patch_response}=    PATCH    ${BASE_URL}${non_existent_id}    
#    ...    headers=${headers}    
#    ...    expected_status=404
#
#    Log To Console   \nResponse: ${patch_response.json()}
#
#Verify Patch Article Request - Title
#    ${headers}=    Create Dictionary    Content-Type=application/json
#    
#    #1. Create new article
#    ${response}=  POST  ${BASE_URL} 
...
#    ${id}=    Set Variable    ${response.json()}[id]
#    
#    #3. Patch only the title of the new article
#    ${new_title}=   Set Variable    Updated Via Patch
#    ${patch_resp}=  PATCH  ${BASE_URL}${id}
#    ...             data={"title": "${new_title}"}  
#    ...             headers=${headers}    expected_status=200
#
#    #3. Verify title change
#    Should Be Equal As Strings    ${patch_resp.json()}[title]      ${new_title}
#    #Verify content hasn't changed (compare against original)
#    Should Be Equal As Strings    ${patch_resp.json()}[content]    ${response.json()}[content]
#
#    ${del_response}=    DELETE    ${BASE_URL}${id}
#    ...    headers=${headers}
#    ...    expected_status=204
#
#    Log To Console    \nSuccessfully created, patched and deleted article ${id}
#
#Verify Patch Article Request - Content
#    ${headers}=    Create Dictionary    Content-Type=application/json
#    
#    #1. Create new article
#    ${response}=  POST  ${BASE_URL} 
...
#    ${id}=    Set Variable    ${response.json()}[id]
#    
#    #3. Patch only the content of the new article
#    ${new_content}=   Set Variable    Updated Via Patch
#    ${patch_resp}=  PATCH  ${BASE_URL}${id}
#    ...             data={"content": "${new_content}"}  
#    ...             headers=${headers}    expected_status=200
#
#    #3. Verify title hasn't change
#    Should Be Equal As Strings    ${patch_resp.json()}[title]      ${response.json()}[title]
#    #Verify content change
#    Should Be Equal As Strings    ${patch_resp.json()}[content]    ${new_content}
#
#    ${del_response}=    DELETE    ${BASE_URL}${id}
#    ...    headers=${headers}
#    ...    expected_status=204
...
#    Log To Console    \nSuccessfully created, patched and deleted article ${id}
#
#Verify Patch Article Request - No Changes
#    ${headers}=    Create Dictionary    Content-Type=application/json
#    
#    #1. Create new article
#    ${response}=  POST  ${BASE_URL} 
...
#    ${id}=    Set Variable    ${response.json()}[id]
#    
#    #3. Call Patch with no changes
#    ${patch_resp}=  PATCH  ${BASE_URL}${id}
#    ...             data={}  
#    ...             headers=${headers}    expected_status=200
#
#    #3. Verify title and content have not changed
#    Should Be Equal As Strings    ${patch_resp.json()}[title]      ${response.json()}[title]
#    #Verify content change
#    Should Be Equal As Strings    ${patch_resp.json()}[content]    ${response.json()}[title]
#
#    ${del_response}=    DELETE    ${BASE_URL}${id}
#    ...    headers=${headers}
#    ...    expected_status=204
